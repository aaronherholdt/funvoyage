      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-6 space-y-6 scroll-smooth mb-32">
               ...
               >
                 ...
                 {!isUser && (
                    <div className="flex items-end gap-2 max-w-[90%]">
                       <div className="w-10 h-10 rounded-full bg-white flex-shrink-0 flex items-center justify-center text-xl shadow-sm border-2 border-slate-100">
                         {theme.aiAvatar}
                       </div>
          ...
          {isListening && (
             <div className="flex justify-center w-full my-4">
                 <div className="bg-black/80 backdrop-blur text-white px-6 py-3 rounded-full animate-pulse shadow-lg max-w-[90%] text-center">
          ...
          <div className="flex flex-col items-center gap-4">
             ...
             <div className={`h-8 flex items-center justify-center space-x-2 transition-opacity duration-300 ${isListening || isSpeaking ? 'opacity-100' : 'opacity-0'}`}>
                ...
                <div
                    ...
                    className={`w-2 rounded-full transition-all duration-100 ${
                        isListening ? 'bg-red-500' : theme.visualizer
                    }`}
            ...
            <button
                ...
                className={`
                    ...
                    ${theme.micButtonSize}
                    ${isListening ? 'bg-red-500 text-white scale-110 shadow-red-200' : theme.button}
                `}
                >
                {isListening ? <MicOff size={theme.micIconSize} /> : <Mic size={theme.micIconSize} />}
            </button>
            
            <p className="text-slate-400 font-medium text-sm pb-2">
            ...
            {theme.showText && (
                <div className="w-full max-w-md opacity-0 hover:opacity-100 focus-within:opacity-100 transition-opacity duration-300">
                    ...
                    <input 
                        ...
                        className="flex-1 rounded-full border border-slate-200 bg-white px-4 py-2 text-sm focus:outline-none focus:border-teal-500 shadow-sm"
                    />
