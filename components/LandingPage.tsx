...
import { Mic, ArrowRight, Lock, Globe, Brain, Sparkles, Shield, Users, Check, X, Crown, Map } from 'lucide-react';
...
export const LandingPage: React.FC<LandingPageProps> = ({ onStart, onLogin }) => {
    ...
    <div className="animate-in fade-in slide-in-from-bottom-8 duration-500">
        <div className="text-center max-w-3xl mx-auto mb-12">
            ...
            <p className="text-xl text-slate-600 leading-relaxed">
                ...
                We combine <span className="font-bold text-slate-800">Inquiry-Based Learning</span> with digital journaling to turn every trip into a lesson in curiosity.
            </p>
        </div>
        ...
        <div className="mt-16 bg-slate-900 rounded-[2.5rem] p-8 md:p-12 max-w-6xl mx-auto relative overflow-hidden">
  ...
  const renderMainGrid = () => (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6 animate-in fade-in duration-500">
        {/* Hero Card */}
        <div className="col-span-1 md:col-span-2 md:row-span-2 bg-slate-900 rounded-[2.5rem] p-8 md:p-12 flex flex-col justify-between relative overflow-hidden bento-card">
           <div className="relative z-10">
              <span className="inline-block px-4 py-1.5 rounded-full bg-teal-500/20 text-teal-300 text-xs font-bold uppercase tracking-wider mb-6 border border-teal-500/30">
                  Voice-First Travel Companion
              </span>
              <h1 className="font-kid text-5xl md:text-7xl text-white leading-tight mb-6">
           ...
           <div className="absolute bottom-0 left-0 w-64 h-64 bg-indigo-600 rounded-full blur-[100px] opacity-30 translate-y-1/3 -translate-x-1/3 pointer-events-none"></div>
        </div>

        {/* 3D Interactive Passport Card */}
        <div className="col-span-1 md:col-span-1 min-h-[320px] bg-orange-50 rounded-[2.5rem] relative group perspective-1000 cursor-pointer flex items-center justify-center overflow-visible bento-card">
            <div className="absolute top-6 left-6 z-0">
                <h3 className="font-bold text-orange-900/50 text-sm uppercase tracking-wider">Collect Badges</h3>
            </div>
            ...
            <div className="absolute bottom-6 text-center w-full opacity-0 group-hover:opacity-100 transition-opacity delay-100 pointer-events-none">
                <p className="text-orange-600 text-xs font-bold">Collect them all!</p>
            </div>
        </div>

        {/* Popping Map Card */}
        <div className="col-span-1 md:col-span-1 min-h-[320px] bg-teal-50 rounded-[2.5rem] relative group overflow-hidden flex flex-col items-center justify-center bento-card">
             <div className="absolute top-6 left-6">
                <h3 className="font-bold text-teal-900/50 text-sm uppercase tracking-wider">Any Destination</h3>
             </div>

             <div className="relative w-40 h-40 flex items-center justify-center">
                 {/* Center Map */}
                 <div className="relative z-10 bg-white w-20 h-20 rounded-3xl shadow-lg flex items-center justify-center text-4xl border-4 border-teal-100 group-hover:scale-90 transition-transform duration-300">
                     🗺️
                 </div>
             ...
             <p className="absolute bottom-8 text-teal-600 font-bold text-sm opacity-60 group-hover:opacity-100 transition-opacity">
                 Hover to explore
             </p>
        </div>

        {/* Age Adaptive Feature Card */}
        <div className="col-span-1 md:col-span-2 bg-white border border-slate-200 rounded-[2.5rem] p-8 flex flex-col md:flex-row items-center gap-8 bento-card">
           <div className="flex-1 space-y-6 w-full">
               <div>
                    <h3 className="font-bold text-slate-900 text-2xl mb-2">Grows with you.</h3>
                    <p className="text-slate-500">Nia adapts her personality, questions, and look based on your child's age.</p>
               </div>
                   ...
                   {[5, 8, 11, 14].map((age) => {
                       ...
                       return (
                           <button
                               ...
                               className={`px-4 py-2 rounded-full text-sm font-bold transition-all ${isActive ? 'bg-slate-900 text-white shadow-md' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}
                           >
                               {label}
                           </button>
                       )
           ...
           {/* Live Preview Bubble */}
           <div className={`w-full md:w-80 p-6 rounded-3xl transition-all duration-500 border-2 border-dashed ${previewTheme.containerBg} border-slate-200`}>
                <div className="flex items-start gap-3">
                    <div className="w-10 h-10 bg-white rounded-full shadow-sm flex items-center justify-center text-xl flex-shrink-0">
                ...
                <div className="mt-4 flex justify-end">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white shadow-sm transition-colors ${previewAge <= 6 ? 'bg-orange-500' : previewAge <= 9 ? 'bg-teal-500' : previewAge <= 12 ? 'bg-blue-500' : 'bg-violet-500'}`}>
                        <Mic size={14} />
                    </div>
                </div>
           </div>
         ...
         <div 
            ...
            className="col-span-1 md:col-span-2 bg-indigo-50 rounded-[2.5rem] p-8 flex items-center justify-between bento-card group cursor-pointer hover:bg-indigo-100 transition-colors"
         >
              <div>
                  <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="font-bold text-indigo-900">Parent Controlled</span>
                  </div>
                  <h3 className="text-xl text-indigo-900/80 font-medium">
                      No ads. Private data. <br/>
                      <span className="font-bold text-indigo-900">100% Safe for kids.</span>
                  </h3>
              </div>
              <div className="hidden md:block opacity-50 group-hover:opacity-100 transition-opacity">
                  <div className="flex -space-x-3">
                      {[1,2,3].map(i => (
                          <div key={i} className="w-10 h-10 rounded-full border-2 border-indigo-50 bg-white flex items-center justify-center text-xs shadow-sm">
                              {['🛡️','🔒','✅'][i-1]}
                          </div>
                      ))}
                  </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans selection:bg-teal-100 pb-12">
      <style>{`
